...
confusion_cell = cell(num_fish,num_regions+1);
randomtime_cell = cell(num_fish,num_regions+1,num_classreps);
randomroi_cell = cell(num_fish,num_regions+1,num_classreps);
...
% collapse the celltype dimension
total_rois = squeeze(sum(total_rois,2));
...
%     for celltype = 1:3
    ...
    af_temp = cell(num_fish,1);
    % for all the fish
    for fish = 1:num_fish
        % for all the cell types
        af_temp{fish} = af_conc_trace(af_fish_ori==fish,:,:,:);
    end

    % for all celltypes
    parfor fish = 1:num_fish
        disp(strcat('Fish:',num2str(fish),'region:',num2str(region)))


        % get the traces
        conc_trace = af_temp{fish};
%         conc_trace = af_conc_trace(af_fish_ori==fish&af_cell_info==celltype-1,:,:,:);
        ...
        for classr = 1:num_classreps
            ...
            [subsampled_traces,random_rois] = subsample_roi(subsampled_traces,roi_flag,region,roi_percentage,total_rois(:,fish));
            ...
            randomtime_cell{fish,region,classr} = random_time;
            randomroi_cell{fish,region,classr} = random_rois;
        end
        % store
        confusion_cell{fish,region} = rep_results;
...
save_name = strjoin({'ClassificationPerFishCombined','classreps',num2str(num_classreps),...
