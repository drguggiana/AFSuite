...
switch flag
    ...
    case 3 % all stimuli split into 4 time bins
        % define the time bin number
        bin_number = 4;
        % get the info from the data_in
        roi_num = size(data_in,1);
...
%         stim_percat = cellfun(@length,cat_index);
        ...
        % get the target number of time bins
        target_time = floor((time_num/bin_number).*0.95);
        % get the indexes for the time
        time_index = [1:time_num/bin_number:time_num;...
            time_num/bin_number:time_num/bin_number:time_num];
...
%         cat_number = length(cat_index)*size(time_index,2);
        ...
        % allocate memory to save the randomization vector
        random_vector = cell(stim_num,size(time_index,2),rep_num);
        ...
        for stim = 1:stim_num
            ...
            for times = 1:size(time_index,2)
                ...
                for reps = 1:rep_num
                    % get the data
                    stim_data = reshape(data_in(:,time_index(1,times):time_index(2,times),...
                        stim,reps),roi_num,[]);
                    % get the random vector
                    random_vector{stim,times,reps} = randperm(size(stim_data,2),target_time);
                    % subsample
                    subsampled_data(:,:,count,reps) = stim_data(:,random_vector{stim,times,reps});
                end
                % update the counter
                count = count + 1;
            end
