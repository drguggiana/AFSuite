...
default_settings(1).font_factor = 4;
...
for ax_count = 1:ax_number
    ...
    % apply the labels
    xlabel(ax,fig_set(ax_count).XLabel)
    ylabel(ax,fig_set(ax_count).YLabel)
    title(ax,fig_set(ax_count).Title,'Interpreter','None')
    ...
    if fig_set(ax_count).colorbar
        ...
        ylabel(cba,fig_set(ax_count).colorbar_label)
    end

    %% Apply the font size
    set(ax,'FontSize',font_size_target*font_factor)
    %% Apply colormap
...
current_size = fig_handle.PaperPosition;
